...
resource "aws_autoscaling_group" "broker_nodes" {
  ...
  max_size         = "${var.separate_zookeeper == "true" ? var.broker_count : "0"}"
  min_size         = "${var.separate_zookeeper == "true" ? var.broker_count : "0"}"
  desired_capacity = "${var.separate_zookeeper == "true" ? var.broker_count : "0"}"
